 ...
 *   - advisories  — NUKIB security advisories and alerts
 *   - frameworks  — NUKIB framework series (National Cybersecurity Framework, ISMS)
 *
...
export function searchGuidance(opts) {
    ...
    const conditions = ["guidance_fts MATCH :query"];
    ...
    return db
        .prepare(`SELECT g.* FROM guidance_fts f
...
export function getGuidance(reference) {
    const db = getDb();
    return (db
        .prepare("SELECT * FROM guidance WHERE reference = ? LIMIT 1")
        .get(reference) ?? null);
}
export function searchAdvisories(opts) {
    const db = getDb();
    const limit = opts.limit ?? 20;
    const conditions = ["advisories_fts MATCH :query"];
    const params = { query: opts.query, limit };
    if (opts.severity) {
    ...
    const where = conditions.join(" AND ");
    return db
        .prepare(`SELECT a.* FROM advisories_fts f
...
export function getAdvisory(reference) {
    const db = getDb();
    return (db
